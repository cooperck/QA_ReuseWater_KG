from KNN_Rwater2Process import *

class QuestionParser:
    def __init__(self):
        self.KNN = KNN_Water2Process()
    '''构建实体节点'''
    def build_entitydict(self, args):
        entity_dict = {}
        for arg, types in args.items():
            ## arg为单元名称“超滤系统”等、工程名称“蒙西污水处理厂”等、水质情况“COD_in”等
            ## types为['unit']、['project']、'各自水质数值'
            for type in types:
    ...
    def parser_main(self, res_classify):
        ...
        # if question_types == 'wquality_process':
        #     entity_dict = args
        if question_types == 'project_unit' or 'unit_project':
            entity_dict = self.build_entitydict(args) #得到问题中的需求对应的字典，肺气肿吃啥，得到的是{unit:[中空纤维超滤,保安过滤器]}等，问题中有多个关系的进行多关系分类存储
        ...
        for question_type in question_types:
            ...
            elif question_type == 'wquality_process':
                # 先将数据输入分类器，查询到最接近的工程名称，分类器返回entity_dict={'project'：[‘项目名称’]}
                # entity_dict为{'recovery': '70', 'quantity': '200', 'CI_in': '1500'}
                # entity_dict_1 ={'project':['蒙西污水处理厂']}
                # print(args)
                entity_dict_1 = self.KNN.KNN_W2P(args)
                sql = self.sql_transfer(question_type, entity_dict_1.get('project'))


            if sql:
                sql_['sql'] = sql #存储问题中一个信息信息{'question_type':'问题类型','sql':[查询语句]}

                sqls.append(sql_) #存储问题中所有的查询信息
